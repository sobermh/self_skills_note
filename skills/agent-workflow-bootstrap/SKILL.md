...
本 Skill 自身是唯一的可复用规则源。目标项目只保留结合该项目实际情况生成的规则、角色文档和 Todo 台账；不要把通用模板目录复制进项目，也不要制造第二套模板源。
...
为项目创建或更新角色说明时，每个角色文档至少包含：身份目标、启动读取顺序、职责、禁止越界、输入、输出、交接条件、停止条件。内容必须结合项目技术栈和目录边界，不能只保留通用口号。
...
- `docs/agents/` 下的 Architect、Developer、UI、QA，以及按需 Beta 角色说明。
...
1. 盘点规则、角色文档、Todo、历史里程碑、结构测试和旧模板目录。
...
6. 从总览删除纯文档维护流水；重要变化归入具体任务、设计决策或归档摘要。
