---
name: agent-workflow
description: Multi-agent engineering workflow harness for project development. Two trigger classes. (1) Governance — initialize (bootstrap), audit, or upgrade a repository's development governance; define Architect/Developer/UI/QA/Beta responsibilities; add Design and release gates; standardize commit planning; create or migrate two-layer Todo and Bug ledgers; enforce defect lifecycle, context-loading, and archive rules. (2) Daily role startup — when the user asks to start or continue a session as Architect, Developer, UI, QA, or Beta (e.g. "以 Developer 角色开工", "作为 QA 验收这张卡", "按工作流开发这个功能"), load the matching role section and the project's workflow delta before working. Do not use for ordinary feature implementation when no role or governance context is requested.
---
...
本 Skill 是跨项目复用的开发工作流引擎，也是**唯一的可复用规则源**：角色职责、流程、台账 schema、Design Gate、提交规划规则都只在这里维护。目标项目不复制通用规则，只保留一份薄 delta 文件（`develop/dev/workflow.md`，见下文契约）记录项目专属边界，以及项目自己的 Todo/Bug 台账。禁止把通用模板目录复制进项目或制造第二套模板源。
...
| 模式 | 触发场景 | 装载 |
|---|---|---|
| **role-startup** | 日常开发：用户要求以某角色开工/验收/设计 | `references/roles.md` 对应角色节 + 项目 delta + 项目规则；创建/更新任务卡时读 `todo-templates.md` 相关节，处理缺陷时读 `bug-ledger.md` |
| **bootstrap** | 新项目建立治理 | `references/bootstrap.md` + `roles.md` + `todo-templates.md` + `bug-ledger.md` |
| **upgrade** | 旧项目补齐/收敛到本工作流 | `references/bootstrap.md`（升级章）+ `todo-templates.md` + `bug-ledger.md` + `checklist.md` |
| **audit** | 只检查治理现状，不改文件 | `references/checklist.md`；审计缺陷治理时再读 `bug-ledger.md`（用户未要求修改时不写文件） |
...
4. 按台账上下文装载规则读当前任务卡（Todo 总览 + 当前里程碑 + 卡显式引用的文档；历史里程碑默认不读）。创建、重开或更新任务卡结构/状态时补读 `references/todo-templates.md` 相关节；处理或发现缺陷时，按 `references/bug-ledger.md` 只读 Bug 总索引、当前里程碑和明确点名的记录。
...
非只读开发、QA 或 Beta 会话中发现达到登记边界的缺陷时，必须按 `references/bug-ledger.md` 在本次交接前登记：任何角色可创建 `open`，Developer 修复后改为 `fixed`，只有独立 QA 可改为 `verified`，接受风险须由 Architect/用户明确决定。Audit 或用户明确要求只读时只报告应登记内容，不写项目文件。Bug 完整事实只在里程碑明细展开，Todo、测试计划和发布清单只引用 ID。
...
- **缺陷不随会话消失**。达到登记边界的 Bug 不得只留在聊天、日志或最终报告；ID 永不复用，修复失败和接受风险都保留历史。
...
- `references/bug-ledger.md` — 两层 Bug 台账、登记边界、编号与严重级别、状态所有权、模板、上下文和守门规则。
- `references/bootstrap.md` — 项目落地文件布局、bootstrap/upgrade 执行要求、完成报告格式。
- `references/checklist.md` — audit/收尾验证清单。

## Changelog

- **1.3.0** — 新增跨项目两层 Bug 台账：统一 `develop/bug/` 默认布局、永久 ID、P0-P3、`open -> fixed -> verified`/`accepted` 状态所有权、记录模板、Todo/测试/Release Gate 联动、最小上下文与结构守门；接入 bootstrap、upgrade、audit 和日常角色流程。
