...
- **提交时机（一卡收口一提交）**：一张卡的逻辑提交在该卡到达提交点时即提请执行，不跨卡攒批。提交点默认：主分支直接工作 → `qa-pass` 后提交（未经 QA 不进主分支历史）；feature branch / worktree 隔离 → 可前移至 `implemented`（QA 把关 merge）。项目 delta 可覆盖。工作树里挂起的未授权提交要在卡备注登记范围；多张卡改动混入同一文件同一区域后再拆分，是本规则明确要避免的高成本失败模式。
